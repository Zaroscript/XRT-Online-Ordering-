...
export function Tabs({
  ...
  selectedTab,
  onTabChange,
}: TabsProps & { selectedTab?: string; onTabChange?: (id: string) => void }) {
  const [internalActiveTab, setInternalActiveTab] = useState(defaultTab);

  // Use controlled state if available, otherwise fall back to internal state
...
export function TabPanel({ id, children, className }: TabPanelProps) {
  ...
  return (
    <div
      ...
      className={cn(isActive ? 'animate-fade-in' : 'hidden', className)}
    >
      {children}
    </div>
  );
