...
import { ReviewIcon } from '@/components/icons/sidebar/review';
import { offset, flip, autoUpdate, useFloating, shift } from '@floating-ui/react';
import { useResendEmailCampaignMutation, useDeleteEmailCampaignMutation } from '@/data/emails';
...
type IProps = {
  emails: EmailCampaign[] | undefined;
...
const ActionMenu = ({ campaign }: { campaign: EmailCampaign }) => {
  const { t } = useTranslation();
  const { mutate: resend, isPending: resending } = useResendEmailCampaignMutation();
  const { mutate: deleteCampaign, isPending: deleting } = useDeleteEmailCampaignMutation();
          ...
          <Menu.Items className="w-48 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
            <div className="py-1">
              ...
              <Menu.Item>
                ...
                {t('common:text-delivered', 'Delivered')}
              </span>
        </button>
                )}
              </Menu.Item>
...
const EmailList = ({ emails, paginatorInfo, onPagination, onSort, onOrder }: IProps) => {
  const { t } = useTranslation();
  ...
  const columns: any = [
    ...
    {
      title: t('form:input-label-subject'),
      dataIndex: 'subject',
      key: 'subject',
      align: alignLeft,
      width: 250,
      render: (subject: string, record: EmailCampaign) => (
        <div>
          <span className="block truncate max-w-xs font-medium text-body-dark">{subject}</span>
          <span className="text-xs text-gray-400">{record.heading}</span>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      width: 150,
      render: (date: string) => (
        <span className="whitespace-nowrap text-sm">{dayjs(date).format('DD MMM YYYY')}</span>
      ),
    },
    {
      ...
      align: alignRight,
      width: 80,
      render: (_: string, record: EmailCampaign) => <ActionMenu campaign={record} />,
    },
  ];
