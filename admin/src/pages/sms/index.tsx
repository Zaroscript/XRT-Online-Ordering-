...
import { useSmsQuery } from '@/data/sms';
...
export default function SmsPage() {
  ...
  const { data, isPending: loading, error } = useSmsQuery(searchTerm);
    ...
    <>
      <Card className="mb-8 flex flex-col items-center xl:flex-row">
        <div className="mb-4 md:w-1/3 xl:mb-0">
          <PageHeading title={t('common:sidebar-nav-item-sms')} />
        </div>
        ...
        <div className="flex w-full flex-col xl:w-2/3 xl:flex-row xl:items-center xl:ms-auto">
          ...
          <LinkButton
            href={Routes.sms.create}
            className="h-12 w-full xl:w-auto xl:ms-6 mt-5 xl:mt-0"
          >
            <span className="hidden xl:block">
              + {t('form:button-label-add')} {t('common:sms')}
            </span>
      ...
      <SmsList campaigns={data?.data} paginatorInfo={data?.paginatorInfo} />
    </>
  );
}

SmsPage.authenticate = {
  permissions: adminOnly,
};
SmsPage.Layout = Layout;
